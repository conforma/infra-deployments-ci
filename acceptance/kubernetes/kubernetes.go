...
func createNamespace(ctx context.Context) (context.Context, error) {
	...
	config, err := clusterState.RestConfig()
	if err != nil {
		return ctx, fmt.Errorf("getting REST config: %w", err)
	}
	...
	pubKeyPath := filepath.Join(testenv.RepoRoot(ctx), "acceptance", "pub.key")
	pubKeyData, err := os.ReadFile(pubKeyPath)
	if err != nil {
		return ctx, fmt.Errorf("reading public key from %s: %w", pubKeyPath, err)
	}
	...
	if pipelineObj != nil {
		...
		pipelineCopy.Namespace = ns.Name
		_, err = tektonClient.Pipelines(ns.Name).Create(ctx, pipelineCopy, metav1.CreateOptions{})
		if err != nil {
...
func createPolicy(ctx context.Context, content *godog.DocString) (context.Context, error) {
	...
	ctx = testenv.SaveState(ctx, *state)
...
func runPipeline(ctx context.Context, params *godog.Table) (context.Context, error) {
	...
	// Wait for completion with a timeout slightly longer than the PipelineRun
	// timeout to allow the PipelineRun to fail naturally with a clear error.
	watchCtx, cancel := context.WithTimeout(ctx, 35*time.Minute)
	defer cancel()

	logger.Log("Waiting for PipelineRun to complete...")
	watcher, err := tektonClient.PipelineRuns(state.namespace).Watch(watchCtx, metav1.SingleObject(metav1.ObjectMeta{
		Name: pr.Name,
	}))
	if err != nil {
	...
	for event := range watcher.ResultChan() {
		...
		if !state.pipelineResult {
			logPipelineRunFailure(ctx, logger, pr, tektonClient, state.namespace)
		}
	...
	if !completed {
		if watchCtx.Err() == context.DeadlineExceeded {
			return ctx, fmt.Errorf("timed out waiting for PipelineRun %s to complete", state.pipelineRunName)
		}
		return ctx, fmt.Errorf("lost watch connection for PipelineRun %s before completion", state.pipelineRunName)
	}
...
// logPipelineRunFailure logs detailed failure information for a PipelineRun,
// including conditions, child TaskRun statuses, and step termination details.
func logPipelineRunFailure(ctx context.Context, logger log.Logger, pr *pipeline.PipelineRun, tektonClient tekton.TektonV1Interface, namespace string) {
	...
	for _, child := range pr.Status.ChildReferences {
		...
		tr, err := tektonClient.TaskRuns(namespace).Get(ctx, child.Name, metav1.GetOptions{})
		if err != nil {
			continue
		}
...
func pipelineShouldFail(ctx context.Context) error {
	...
	if state.pipelineResult {
		return fmt.Errorf("pipeline succeeded, expected failure. TEST_OUTPUT: %s", state.testOutput)
	}
