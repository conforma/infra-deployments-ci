...
// SaveState stores state back into the context. Use this when updating
// state that was previously retrieved via FetchState.
func SaveState[T State](ctx context.Context, state T) context.Context {
	return context.WithValue(ctx, state.Key(), state)
}
